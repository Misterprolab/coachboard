...
const BASE_CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .logo-placeholder { width: 120px; height: 120px; border-radius: 60px; background: #0E5A3C; color: #D4AF37; font-size: 54px; font-weight: 900; display: flex; align-items: center; justify-content: center; }
  .logo-img { width: 120px; height: 120px; border-radius: 14px; object-fit: contain; }
...
export function exportTacticalPdf(header: PdfHeader, data: TacticalPdfData) {
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Campo Tattico</title><style>${BASE_CSS}</style></head><body>
    ...
    <div class="pitch-wrap">${data.svgContent}</div>
  </body></html>`;
  _printHtml(html);
}
